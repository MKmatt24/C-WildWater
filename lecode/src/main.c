...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        fprintf(stderr, "Usage: %s <input.csv> [mode] [output/factory_id] ...\n", argv[0]);
        return 1;
    }
    ...
    for (int i = 2; i < argc; i++) {
        if (strcmp(argv[i], "max") == 0 && (i + 1) < argc) {
            ...
            if (!f_data) {
                ...
                // Parcourir le fichier jusqu'à ce qu'il n'y ait plus de nouveaux nœuds
                ...
                while (nodes_added && iteration < max_iterations) {
                    ...
                    while (fgets(line, sizeof(line), data_file)) {
                        ...
                        // Capture tous les tronçons en aval de l'usine
